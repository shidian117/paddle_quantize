...
add_arg('num_epochs',               int,    5,                                           "The number of total epochs.")
...
parser.add_argument('--step_epochs', nargs='+', type=int, default=[15,25], help="piecewise decay step")
# yapf: enable
...
def compress(args):
    if args.data == "cifar10":
        transform = T.Compose([ T.RandomCrop(32, padding=4),T.RandomHorizontalFlip(), T.Transpose(),  T.Normalize((0.4914, 0.4822, 0.4465), (0.2470, 0.2435, 0.2616))])
        ...
        quanter.save_quantized_model(
            ...
            input_spec=[
                paddle.static.InputSpec(
                    shape=[None, 3, 32, 32], dtype='float32')
            ])
        '''
        path = os.path.join(args.model_save_dir, "inference_model", 'qat_model')
        paddle.jit.save(net, path, input_spec=[paddle.static.InputSpec(shape=[None, 3, 32, 32], dtype='float32')])
